...
public class RedStopWindowBraintree extends StopWindow{
	private String BACKGROUND = "src/UI/RedStopWindow.png";
	List<Predictions> braintree;
	List<Predictions> alewife;
	BufferedImage bg;
	public RedStopWindowBraintree(String name, MapWindow panel){
		super(name, panel);
		try {
	...
	void generateLabels() {
		...
		add(nameLabel);
		braintree = ((MapWindow) parent).getRedPredictions(stopName, "Braintree");
	...
	private void fillLabels(){
		...
		for(int j = 0; j < 3; j++){
			...
			for(int i = 0; i < braintree.size(); i++){
				if((braintree.get(i).getSeconds()< smallestInt 
						&& indexVisitedA.indexOf(braintree.get(i).getSeconds()) == -1)
						|| smallestInt == -1){
					smallestInt = braintree.get(i).getSeconds();
